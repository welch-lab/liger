...
```{r, results='hide'}
library(rliger)
# Together load other necessary libraries
# `dplyr` is a nice package for data table manipulation
library(dplyr)
# `cowplot` is a common package for arranging multiple plots
library(cowplot)

pbmcLiger <- importPBMC()
```

For creating a liger object from raw counts data or any other types of source, please refer to the [detailed tutorial for importing data](import_data.html).
