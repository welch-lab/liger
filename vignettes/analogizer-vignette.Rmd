...
analogy = selectGenes(analogy,var.thresh = 0.1)
...
plotGene(analogy,gene="Malat1")
plotGeneViolin(analogy,gene="Malat1")
```
...
```r
suggestK(analogy) # plot entropy metric to find an elbow that can be used to select the number of factors
suggestLambda(analogy,k) # plot alignment metric to find an elbow that can be used to select the value of lambda
```
