...
#' consensus of the factorized matrices factor gene loading matrix \eqn{W} and
#' \eqn{V} matrices for each dataset. The cell factor loading \eqn{H} matrices
#' are eventually solved with the consensus \eqn{W} and \eqn{V} matrices.
#'
...
#' initialization is recommended). Default \code{1}.
...
.runCINMF.list <- function(
    ...
    #
    msg <- "Solving last round of NNLS problems"
    ...
    # }
    # W <- inmf_solveW(Hs, NULL, Vs, object, lambda, nCores = nCores)
    # objErr <- sum(sapply(seq_along(object), function(i)
    #     inmf_objErr_i(H = Hs[[i]], W = W, V = Vs[[i]], E = object[[i]],
    #                   lambda = lambda)
    # ))
    out <- .runINMF.list(object, k = k, lambda = lambda, nIteration = 30,
